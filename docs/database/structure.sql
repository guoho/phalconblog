...
CREATE TABLE IF NOT EXISTS tag(
  tag_id smallint(6) unsigned auto_increment comment '标签ID',
...
CREATE TABLE IF NOT EXISTS tag_relation(
  id int(11) unsigned auto_increment comment '自增长ID',
  post_id int(11) unsigned not null default '0' comment '文章ID', 
  tag_id smallint(6) unsigned not null default 0 comment '分类ID', 
  primary key(id),
  index post_id(post_id),
  index tag_id(tag_id)
